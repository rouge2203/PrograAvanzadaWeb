...
namespace API.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        {
            var res = await new BE.Products(dbcontext).GetAllAsync();
        ...
        {
            var products = await new BE.Products(dbcontext).GetOneByIdAsync(id);
        ...
        public async Task<IActionResult> PutProducts(int id, models.Products products)
        {
            ...
            {                
                ...
                new BE.Products(dbcontext).Update(mapaux);
                }
            catch (Exception ee)
            {
        ...
        public async Task<ActionResult<models.Products>> PostProducts(models.Products products)
        {
            ...
            {
                var mapaux = mapper.Map<models.Products, data.Products>(products);
                new BE.Products(dbcontext).Insert(mapaux);
                }
            catch (Exception)
            {
                BadRequest();
        ...
        {
            var products = new BE.Products(dbcontext).GetOneById(id);
            var mapaux = mapper.Map<data.Products, models.Products>(products);
            if (products == null)
            {
                return NotFound();
            ...
            try
            {
                new BE.Products(dbcontext).Delete(products);
            }
            catch (Exception)
            {
                BadRequest();
        ...
        {
            return (new BE.Products(dbcontext).GetOneById(id) != null);
        }
    }
}
